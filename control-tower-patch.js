...
function ensureHeroKpis(){
  ...
  if(!$('ctNeedAbs')){
    const card=document.createElement('div');card.className='ct-kpi ct-kpi-bad';card.innerHTML='<div class="l">연동이 필요한 도도매처수</div><div class="v" id="ctNeedAbs">-</div><div class="s" id="ctNeedAbsSub">실사업자번호 기준 연동 필요 절대처수</div>';grid.appendChild(card);
  }
  if(!$('ctSupply')){
    const card=document.createElement('div');card.className='ct-kpi ct-kpi-info';card.innerHTML='<div class="l">도도매 공급처수</div><div class="v" id="ctSupply">-</div><div class="s" id="ctSupplySub">현재 공급 중인 도도매 절대처수</div>';grid.appendChild(card);
  }
...
function syncHeroSupportKpis(){
  const copy=(src,dst)=>{const a=$(src),b=$(dst);if(a&&b)b.textContent=a.textContent||'-';};
  copy('kpiAbs','ctNeedAbs');copy('kpiSupply','ctSupply');
...
function trimDuplicateChangeCards(changes){
  if(!changes)return;
  const duplicateLabels=new Set(['연동률 변화','X→O 개선','O→X 연동 해제']);
  changes.querySelectorAll('.change-card').forEach(card=>{const label=card.querySelector('.change-label')?.textContent.trim();if(duplicateLabels.has(label))card.remove();});
...
function installLayout(){
  ...
  if(oldDetail){const body=oldDetail.querySelector('.ct-detail-body');if(body){[...body.children].forEach(el=>main.insertBefore(el,oldDetail));}oldDetail.remove();}
  ...
  if(aging)aging.id='ctAging';if(ai)ai.id='ctAiSection';if(risk)risk.id='ctRisk';if(action)action.id='ctAction';
  if(changes){changes.id='ctChanges';trimDuplicateChangeCards(changes);}
  if(need)need.id='ctNeed';
  if(legacyKpis)legacyKpis.classList.add('ct-source-kpis');

  let anchor=filters;
  [changes,group,outlet,aging,ai,risk,action,need].forEach(el=>{if(!el)return;anchor.insertAdjacentElement('afterend',el);anchor=el;});
  if(legacyKpis)main.appendChild(legacyKpis);
  ...
  nav.innerHTML='<button data-jump="ctChanges">전월 대비 변화</button><button data-jump="ctGroup">그룹사별 현황</button><button data-jump="ctOutlet">업체별 현황</button><button data-jump="ctAging">Aging & Target</button><button data-jump="ctRisk">Risk TOP 20</button><button data-jump="ctAction">Action Board</button><button data-jump="ctNeed">연동 필요 리스트</button>';
  ...
  syncHeroSupportKpis();
  ['kpiAbs','kpiSupply','kpiAbsSub','kpiSupplySub','ctDelta'].forEach(id=>{const node=$(id);if(node)new MutationObserver(syncHeroSupportKpis).observe(node,{childList:true,subtree:true,characterData:true});});
  ['manager','outlet','month'].forEach(id=>$(id)?.addEventListener('change',()=>setTimeout(syncHeroSupportKpis,80)));
}
...
function ensureActionControls(){
  const panel=$('ctAction'),tbody=$('ctActionBody');if(!panel||!tbody)return;const head=panel.querySelector('.section-head');if(!head)return;panel.classList.add('ct-action-lazy','ct-action-collapsed');
  if(!$('ctActionToggle')){const btn=document.createElement('button');btn.id='ctActionToggle';btn.className='ct-btn ct-action-toggle';btn.type='button';btn.textContent='Action Board 열기';head.appendChild(btn);btn.addEventListener('click',()=>actionOpen?closeActionBoard():openActionBoard());}
  const tableWrap=tbody.closest('.table-scroll');if(tableWrap&&!$('ctActionMoreWrap')){const more=document.createElement('div');more.id='ctActionMoreWrap';more.className='ct-action-more';more.innerHTML='<span id="ctActionShown"></span><button class="ct-btn" id="ctActionMore" type="button">50건 더 보기</button><button class="ct-btn" id="ctActionAll" type="button">전체 보기</button>';tableWrap.insertAdjacentElement('afterend',more);$('ctActionMore')?.addEventListener('click',()=>{actionLimit+=PAGE_SIZE;renderAllXActionBoard();});$('ctActionAll')?.addEventListener('click',()=>{actionLimit=Number.MAX_SAFE_INTEGER;renderAllXActionBoard();});}clearActionRows();
}
...
function renderAllXActionBoard(){
  const api=window.DCMActionSync,tbody=$('ctActionBody');if(!actionOpen||!api?.allRisk||!tbody)return;const rows=api.allRisk(),visible=rows.slice(0,actionLimit);const table=tbody.closest('table'),head=table?.querySelector('thead tr');
  ...
  const panel=tbody.closest('.ct-panel'),desc=panel?.querySelector('.section-head p');if(desc)desc.textContent=`현재 필터 범위에서 X가 하나라도 있는 전체 거래처 ${rows.length}건 · Risk Score 높은 순`;const reasons=window.DCM_ACTION_REASONS||{};
